...
public class AbilityManufacturingUI : MonoBehaviour
{
    ...
    public void SelectRecipe(RecipeData recipe)
    {
        ...
        // Check if we have sufficient inputs before switching UI
        if (recipe.inputs.Count == 0 || SessionState.Instance.stashSalvage < recipe.inputs[0].amount)
        {
            Debug.LogWarning("Insufficient inputs for recipe");
            return;
        }
        ...
        recipeDetailsLabel.text = $"Salvage: {recipe.inputs[0].amount} | Time: {recipe.craftTime}s";
